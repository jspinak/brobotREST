...
public class Find {
    ...
    public Match findFirstMatch(float similarity, StateImageData... stateImageData) {
        return findFirstMatch(similarity, Arrays.asList(stateImageData));
    }

    public Match findFirstMatch(float similarity, List<StateImageData> stateImages) {
        Match match;
        for (StateImageData stateImageData : stateImages) {
            match = findBestMatch(stateImageData.getSearchRegion(), stateImageData.getImage());
